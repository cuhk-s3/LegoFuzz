...
def process_c_files(src, dst, client, max_files=None):
        ...
        for file in c_files:
            with count_lock:
                if max_files is not None and count >= max_files:
                    break
            futures.append(
                executor.submit(process_single_item, file, dst, client, max_files)
            )
    ...
    result_file = os.path.join(dst, "results.jsonl")
...
def main():
    parser = argparse.ArgumentParser(
        description="Process a directory of C files and generate transformed C files in parallel."
    )
    parser.add_argument(
        "--src",
        type=str,
    ...
    parser.add_argument(
        "--dst", type=str, required=True, help="Directory to save generated C files"
    )
    parser.add_argument(
        "--model",
        type=str,
        ...
        required=True,
        help="Which LLM model to use (openai, deepseek, togetherai)",
    )
    parser.add_argument(
        "--max_files",
        type=int,
        default=None,
        help="Maximum number of C files to process",
    )
