...
def extract_one_file(src_file):
    ...
    # --mode rename
    ret, _ = run_cmd(f'{FUNCTION_EXTRACTOR_PATH} --mode rename {tmp_src_file_path} -- -w {CC_ARGS}')
    # --mode rename-global
    ...
    if not is_interesting_function(extracted_json["function"]):
        ...
        if len(matched_lego_name) > 0:
            ...
            matched_lego_name = 'lego_' + ''.join(random.choice(string.ascii_uppercase + string.ascii_lowercase + string.digits) for _ in range(5))
        while True:
