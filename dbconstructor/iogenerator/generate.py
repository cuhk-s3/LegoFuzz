...
def generate_io(input_func: Function)->Function:
    ...
    while num_generated < NUM_IO:
        ...
        num_generated += 1
        if new_func is not None and 'lego_proxy' in new_func.function_body: # we do not support more than 1 NUM_IO for functions with proxy because each time we call iogenerator.generate(input_func), the proxy function would change.
            break
    if len(io_list) != 0:
